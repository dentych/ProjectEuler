package me.tychsen.problems;

import java.util.LinkedList;
import java.util.Queue;

public class Problem8 implements EulerProblem {
    private static final int ADJACENT_NUM = 13;
    private static final String SERIES = "";

    @Override
    public String run() {
        Queue<Integer> queue = new LinkedList<>();
        fillQueue(SERIES, queue);
        long maxProduct = calculateProduct(queue);
        for (int i = ADJACENT_NUM; i < SERIES.length(); i++) {
            queue.poll();
            int num = SERIES.charAt(i) - 48;
            queue.add(num);
            long product = calculateProduct(queue);
            if (product > maxProduct) {
                maxProduct = product;
            }
        }
        return String.valueOf(maxProduct);
    }

    @Override
    public String getName() {
        return "Problem8";
    }

    private void fillQueue(String series, Queue<Integer> queue) {
        for (int i = 0; i < ADJACENT_NUM; i++) {
            int num = series.charAt(i) - 48;
            queue.add(num);
        }
    }

    private long calculateProduct(Queue<Integer> queue) {
        long product = 1;

        for (int item : queue) {
            product *= item;
        }

        return product;
    }
}
